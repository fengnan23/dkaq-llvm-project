...
namespace tidy {
namespace dkaq {
class DefineUndefPairCheck : public DkaqClangTidyCheck {
public:
  ...
  void registerMatchers(ast_matchers::MatchFinder *Finder) override {
    Finder->addMatcher(ast_matchers::functionDecl().bind("function"), this);
  }
  ...
  void check(const ast_matchers::MatchFinder::MatchResult &Result) override {
    if (const auto *Func = Result.Nodes.getNodeAs<FunctionDecl>("function")) {
      ...
      for (const auto &Macro : ActiveMacros) {
        diag(Macro.getValue(),
             "宏定义define'%0' 在函数中和undef必须同时出现. 由DKAQ提供")
            << Macro.getKey();
      }
...
private:
  ...
  public:
    DefineUndefPairPPCallbacks(DefineUndefPairCheck *Check,
    ...
    void MacroUndefined(const Token &MacroNameTok, const MacroDefinition &MD,
      ...
      if (!Check->ActiveMacros.erase(MacroName)) {
        Check->diag(MacroNameTok.getLocation(),
                    "Macro '%0' is undefined without a corresponding #define "
                    "in function")
            << MacroName;
      }
    }
