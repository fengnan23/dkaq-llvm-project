...
namespace tidy {
namespace dkaq {
class DefineRedefinitionCheck : public DkaqClangTidyCheck {
public:
...
private:
  ...
  public:
    DefineRedefinitionPPCallbacks(DefineRedefinitionCheck *Check)
        : Check(Check) {}
    ...
    void MacroDefined(const Token &MacroNameTok,
      ...
      if (DefinedMacros.count(MacroName)) {
        Check->diag(MacroNameTok.getLocation(),
                    "违反GJB R-1-1-5 宏定义在undef前面重复定义 '%0'. 由DKAQ提供")
            << MacroName;
