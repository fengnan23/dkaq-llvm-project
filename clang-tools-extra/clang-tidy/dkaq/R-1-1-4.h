...
namespace tidy {
namespace dkaq {
class KeywordVariableCheck : public DkaqClangTidyCheck {
public:
    ...
    if (const auto *Var = Result.Nodes.getNodeAs<VarDecl>("var")) {
      ...
      if (isCppKeyword(VarName)) {
        diag(Var->getLocation(),
             "违反GJB R-1-1-4 变量名称 '%0' 是c++关键字. 由DKAQ提供")
            << VarName;
      }
    }
  }
...
private:
  bool isCppKeyword(StringRef Name) {
    static const llvm::StringSet<> Keywords = {
        "cout", "cin",    "int",   "unsigned", "if",  "else", "while",
        "for",  "return", "class", "cout",     "cin", "endl"};
