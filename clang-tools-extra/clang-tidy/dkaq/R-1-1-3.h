...
// 希望每一个typedef的内容不能被重复定义,例如:typedef int mytype; typedef float
// mytype.
// 这里的mytype被重复定义,即使这两个typedef在不同的函数作用域中,也需要报错.
namespace clang {
namespace tidy {
namespace dkaq {
class TypedefRedefinitionCheck : public DkaqClangTidyCheck {
public:
  TypedefRedefinitionCheck(StringRef Name, ClangTidyContext *Context)
      : DkaqClangTidyCheck(Name, Context) {}

  void registerMatchers(ast_matchers::MatchFinder *Finder) override {
    Finder->addMatcher(ast_matchers::typedefDecl().bind("typedef"), this);
  }
    ...
    if (const auto *Typedef = Result.Nodes.getNodeAs<TypedefDecl>("typedef")) {
      ...
      for (const auto *Decl : Context->getTranslationUnitDecl()->decls()) {
        if (const auto *OtherTypedef = dyn_cast<TypedefDecl>(Decl)) {
          if (OtherTypedef != Typedef &&
              OtherTypedef->getName() == TypedefName) {
            diag(Typedef->getLocation(),
                 "违反GJB R-1-1-3 重复typedef定义 '%0'. 由DKAQ提供")
                << TypedefName;
